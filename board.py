...
class Board:
    ...
    def remove(self, pieces):  # Time Complexity: O(m*n). The remove() function goes through each row and column (m*n) and checks its conditions based on that location layout
        for piece in pieces:
